...
export interface PreflightReport {
  ...
  ok: boolean;
  summary: { checked: number; failed: number; droppedSkills: number };
...
interface PlatformProbe {
  label: string;
  category: string;
  /** Probe the dependency. `extensionOk` is the cached `bnbot status`
   *  verdict so browser-driven probes don't each re-run it. */
  probe: (extensionOk: boolean) => Promise<ProbeResult>;
}
...
interface BnbotRun {
  ...
  stdout: string;
  stderr: string;
  /** true when no bnbot binary could be spawned at all. */
  missing: boolean;
}

function runBnbot(args: string[], timeoutMs: number): Promise<BnbotRun> {
...
async function probeExtensionLink(): Promise<ProbeResult> {
...
async function codexCdpStatus(extraArgs: string[], timeoutMs: number): Promise<CodexCdpStatus> {
  const r = await runBnbot(["codex", "status", ...extraArgs], timeoutMs);
  if (r.missing) return { missing: true, connected: false, processRunning: false };
 ...
 */
function quitCodexGracefully(): Promise<boolean> {
  return new Promise((resolve) => {
    execFile(
      "osascript",
...
function codexProcessAlive(): Promise<boolean> {
  return new Promise((resolve) => {
...
function codexHealRecentlyFailed(): boolean {
  ...
  if (!last.reason?.includes("CDP")) return false;
...
async function probeCodex(): Promise<ProbeResult> {
  ...
  if (first.connected) return { status: "ok" };
  ...
  if (first.processRunning) {
    ...
    for (let i = 0; i < 8; i++) {
      await sleep(1000);
      if (!(await codexProcessAlive())) { gone = true; break; }
    }
    if (!gone) {
      return {
        status: "failed",
  ...
  if (launched.connected) return { status: "ok" };
  for (let i = 0; i < 6; i++) {
    await sleep(2000);
...
async function probeChatGpt(): Promise<ProbeResult> {
  const r = await runBnbot(["chatgpt", "status"], 8000);
  if (r.missing) {
    return { status: "failed", reason: "未找到 bnbot 命令", hint: "安装 @bnbot/cli" };
  }
  const j = parseJson(r.stdout);
  if (j && j.running === true) return { status: "ok" };
  return {
    status: "failed",
...
const DEFAULT_LOGGED_OUT =
  /login|signin|sign[_-]?in|passport|\/sso\b|accounts\.google\.com|auth\.openai\.com/i;

interface LoginProbeDef {
  label: string;
  /** A page that REQUIRES login — a logged-out hit redirects to a login URL. */
  url: string;
  /** Override the logged-out URL signature for sites with odd redirects. */
  loggedOut?: RegExp;
  /** Navigation timeout (ms). Default 18s; slow apps (Flow) need more. */
  navTimeoutMs?: number;
}
 ...
 */
async function probeLoginByNavigation(def: LoginProbeDef): Promise<ProbeResult> {
...
function navProbe(def: LoginProbeDef): PlatformProbe["probe"] {
  return async (extensionOk: boolean): Promise<ProbeResult> => {
    if (!extensionOk) {
      return {
        status: "failed",
        reason: "BNBot 浏览器扩展未连接",
...
async function probeX(extensionOk: boolean): Promise<ProbeResult> {
  if (!extensionOk) {
    return {
      status: "failed",
      reason: "BNBot 浏览器扩展未连接",
  ...
  const j = parseJson(r.stdout);
  const handle =
    j && (j.username || j.handle || j.screen_name || j.screenName || j.name);
...
function needsExtension(prefixes: Set<string>): boolean {
  for (const p of prefixes) {
    const probe = PLATFORM_PROBES[p];
    if (probe && probe.category !== "local-app") return true;
  }
  return false;
}

function prefixOf(skillId: string): string {
 ...
 */
export async function runPreflight(skills: string[]): Promise<PreflightOutcome> {
  // Count skills per platform prefix so the report can show "3 skills off".
  const skillsByPrefix = new Map<string, string[]>();
  for (const s of skills) {
    const p = prefixOf(s);
    const list = skillsByPrefix.get(p);
    if (list) list.push(s);
    else skillsByPrefix.set(p, [s]);
  }

  const probedPrefixes = new Set(
    [...skillsByPrefix.keys()].filter((p) => PLATFORM_PROBES[p]),
  );
  ...
  const platforms: Record<string, PlatformPreflight> = {};
  ...
  for (const prefix of probedPrefixes) {
    ...
    console.log(
      `[preflight] ${prefix} (${def.label}): ${result.status}` +
        (useCache ? " (cached)" : "") +
...
export function writePreflightReport(report: PreflightReport): void {
  try {
    ...
    console.error(
      `[preflight] failed to write report: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
...
function readPreviousReport(): PreflightReport | null {
  try {
    if (!existsSync(PREFLIGHT_FILE)) return null;
    return JSON.parse(readFileSync(PREFLIGHT_FILE, "utf-8")) as PreflightReport;
  } catch {
    return null;
  }
}
