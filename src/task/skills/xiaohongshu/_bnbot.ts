...
async function runBnbot(args: string[]): Promise<unknown> {
  try {
    ...
    if (e.stderr && e.stderr.trim()) throw new Error(`bnbot xiaohongshu failed: ${e.stderr.trim()}`);
    throw err;
  }
}
...
export type XhsArgs = Record<string, string | number | undefined>;

export async function bnbotXHS(command: string, positional: string[] = [], a: XhsArgs = {}): Promise<unknown> {
