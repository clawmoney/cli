...
export function makeLinkedInSkill(label: string, runner: (input: Input) => Promise<unknown>): SkillHandler {
  return {
    price_usd: 0.002,
    async run(input, ctx) {
      const i = (input ?? {}) as Input;
      ctx.report({ stage: "launching", percent: 5, note: label });
      const stop = startProgressTicker(ctx, "bnbot linkedin working...");
      try {
