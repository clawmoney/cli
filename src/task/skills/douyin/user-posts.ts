...
import { bnbotDYUserPosts } from "./_bnbot.js";
import { startProgressTicker } from "../tiktok/_helpers.js";

export const dyUserPostsSkill: SkillHandler = {
  price_usd: 0.001,
  async run(input, ctx) {
    const i = (input ?? {}) as {
      secUid?: string;
    ...
    ctx.report({
      stage: "launching",
      percent: 5,
      note: `douyin posts of ${secUid}`,
    });
    const stop = startProgressTicker(ctx, "bnbot working...");
    try {
      const raw = await bnbotDYUserPosts({ secUid, cursor, limit });
