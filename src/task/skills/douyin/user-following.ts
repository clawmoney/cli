...
import { bnbotDYUserFollowing } from "./_bnbot.js";
import { startProgressTicker } from "../tiktok/_helpers.js";

export const dyUserFollowingSkill: SkillHandler = {
  price_usd: 0.0015,
  async run(input, ctx) {
    const i = (input ?? {}) as {
      secUid?: string;
    ...
    ctx.report({
      stage: "launching",
      percent: 5,
      note: `douyin following of ${secUid}`,
    });
    const stop = startProgressTicker(ctx, "bnbot working...");
    try {
      const raw = await bnbotDYUserFollowing({ secUid, max_time, limit });
