...
import { bnbotFlowVideoGenerate } from "./_bnbot.js";
...
export const flowVideoGenerateSkill: SkillHandler = {
  price_usd: 1.0,
  async run(input, ctx) {
    ...
    ctx.report({ stage: "launching", percent: 5, note: "Driving Flow video generation..." });
    const stop = startProgressTicker(ctx, "Flow video generation working...");
    try {
      const raw = await bnbotFlowVideoGenerate({
        prompt,
        aspect: str(i, ["aspect", "aspect_ratio", "aspectRatio", "ratio"]),
        duration: num(i, ["duration", "duration_s", "durationSeconds"]),
      ...
      if (result.success === false) {
        throw new Error(result.error || "Flow video generation failed");
      }
      if (responseFormat === "url") {
        ctx.report({ stage: "uploading", percent: 90, note: "Uploading generated video..." });
        const uploaded = await uploadMediaResult(raw, "videos");
        ctx.report({ stage: "done", percent: 100, note: "Video generated and uploaded" });
        return uploaded;
      }
