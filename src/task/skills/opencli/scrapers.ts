...
const limit = (i: Input, fallback?: number) => num(i, ["limit", "count"]) ?? fallback;

// --- Amazon (Apify Amazon Scraper equivalent) ---
export const amazonProductSkill = makeOpenCliSkill("amazon product", (i) =>
  bnbotCommand(["amazon", "product"], [reqStr(i, ["input", "asin", "url", "query"], "input")]),
);
// bnbot has no `amazon offer` yet — keep this one on opencli until migrated.
export const amazonOfferSkill = makeOpenCliSkill("amazon offer", (i) =>
  opencliCommand(["amazon", "offer"], [reqStr(i, ["input", "asin", "url"], "input")]),
);
export const amazonSearchSkill = makeOpenCliSkill("amazon search", (i) =>
  bnbotCommand(["amazon", "search"], [reqStr(i, ["query", "keyword", "q"], "query")], { limit: limit(i) }),
);

// --- Google Maps (Apify Google Maps Scraper equivalent — their largest, 361K) ---
export const ggMapsSkill = makeOpenCliSkill("google maps", (i) =>
  bnbotCommand(["google", "maps"], [reqStr(i, ["query", "keyword", "q"], "query")], { limit: limit(i) }),
);
...
export const webReadSkill = makeOpenCliSkill("web read", (i) =>
  bnbotCommand(["web", "read"], [reqStr(i, ["url"], "url")]),
);
