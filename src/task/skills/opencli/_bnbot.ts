...
function addFlag(args: string[], flag: string, value: FlagValue): void {
...
export async function bnbotCommand(
  base: string[],
  positional: string[] = [],
  ...
  try {
    ...
    if (e.stderr && e.stderr.trim()) throw new Error(`bnbot failed: ${e.stderr.trim()}`);
    throw err;
  }
}
 ...
 */
export async function opencliText(args: string[]): Promise<string> {
  ...
  try {
    ...
    if (!stdout.trim()) throw new Error("opencli returned empty stdout");
    return stdout;
  } catch (err) {
    const e = err as NodeJS.ErrnoException & { stderr?: string };
    if (e.stderr && e.stderr.trim()) throw new Error(`opencli failed: ${e.stderr.trim()}`);
    throw err;
  }
}

export async function opencliCommand(
  base: string[],
  positional: string[] = [],
  flags: Record<string, FlagValue> = {},
): Promise<unknown> {
