...
export const xSearchSkill: SkillHandler = {
  ...
  async run(input, ctx) {
    ...
    if (!q) throw new Error("missing 'q' (or 'query') in input");

    const limit = Math.max(1, Math.min(100, raw.limit ?? 20));
    const tab = raw.tab ?? "top";

    const args = ["x", "scrape", "search", q, "-l", String(limit), "-t", tab];
    if (raw.from) args.push("--from", raw.from);
    if (raw.since) args.push("--since", raw.since);
    if (raw.until) args.push("--until", raw.until);
    if (raw.lang) args.push("--lang", raw.lang);
    if (typeof raw.minLikes === "number") args.push("--minLikes", String(raw.minLikes));
    if (typeof raw.minRetweets === "number") args.push("--minRetweets", String(raw.minRetweets));
    if (raw.has) args.push("--has", raw.has);
