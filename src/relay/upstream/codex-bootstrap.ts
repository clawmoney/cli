...
export function hasCodexFingerprint(): boolean {
...
export async function bootstrapCodexFingerprint(
  ...
  if (hasCodexFingerprint()) {
    throw new Error(
      "codex-fingerprint.json already exists — delete it to re-bootstrap"
    );
  }
  ...
  return new Promise<void>((resolve, reject) => {
    const timer = setTimeout(() => {
      ...
      cleanup();
      reject(
        new Error(`codex fingerprint capture timed out after ${timeoutMs}ms`)
      );
    }, timeoutMs);
    ...
    setTimeout(() => {
      ...
      codexChild.on("error", (err: Error) => {
        ...
        reject(
          new Error(
            `failed to spawn codex: ${err.message} (is the codex CLI installed and in PATH?)`
          )
        );
      });
      ...
      codexChild.on("exit", (code) => {
        ...
        setTimeout(() => {
          if (done || hasCodexFingerprint()) return;
          done = true;
          clearTimeout(timer);
          cleanup();
          ...
          reject(
            new Error(
              `codex -p hi exited with code ${code ?? "unknown"} before the capture proxy saw a /v1/responses upgrade.${detail}`
            )
          );
        }, 800);
      });
