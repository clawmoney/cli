...
interface SearchSkill {
  ...
  category?: string;
  price?: number;
  skill_type?: string;  // "instant" or "escrow"
  avg_rating?: number;
  total_calls?: number;
...
export async function hubCallCommand(options: CallOptions): Promise<void> {
  ...
  try {
    ...
    if (skillType === "escrow") {
      ...
      if (!gigResp.ok) {
        const detail = (gigResp.data as Record<string, unknown>)?.detail ?? gigResp.data;
        spinner.fail(chalk.red(`Failed to create task: ${JSON.stringify(detail)}`));
        process.exit(1);
      }
      const task = gigResp.data as Record<string, unknown>;
      ...
      if (options.pay) {
        ...
        try {
          ...
          const res = await x402Fetch(
            wallet,
            `https://pay.clawmoney.ai/market/escrow/${taskId}?price=${budget}`,
            { method: "POST" }
          );
          if (!res.ok) {
    ...
    if (options.pay) {
      ...
      try {
        ...
        payData = (await res.json()) as Record<string, unknown>;
      } catch (err) {
...
export async function hubSkillsCommand(): Promise<void> {
  ...
  try {
    ...
    const activeSkills = skills.filter((skill) =>
      skill.is_active !== undefined ? skill.is_active : skill.status === "active"
    );
    const hiddenByAgentStatus = activeSkills.filter((skill) =>
      skill.agent_status !== undefined && skill.agent_status !== "active"
    );
    ...
    if (hiddenByAgentStatus.length > 0 || hiddenByOfflineAgent.length > 0) {
      ...
      if (hiddenByAgentStatus.length > 0) {
        const statuses = Array.from(
          new Set(hiddenByAgentStatus.map((skill) => skill.agent_status ?? "-"))
        ).join(", ");
        console.log(
          chalk.dim(
            `  Agent status: ${statuses}. Complete/repair agent claiming before expecting inbound paid calls.`
          )
        );
      }
      if (hiddenByOfflineAgent.length > 0) {
        console.log(
          chalk.dim(
            '  Agent is offline for one or more skills. Run "clawmoney market status" or restart the provider.'
          )
        );
      }
    }
