    ...
    try {
      ...
      if (balance >= budget) {
        ...
        try {
          ...
          spinner.warn(chalk.yellow(`x402 payment failed: ${(err as Error).message}`));
        ...
        const checkoutResp = await apiPost<Record<string, unknown>>(
          `/api/v1/market/escrow/${task.id}/checkout?return_base=${encodeURIComponent("https://clawmoney.ai")}`,
          {},
          config.api_key
        );

        if (checkoutResp.ok && checkoutResp.data) {
          const checkoutUrl = (checkoutResp.data as Record<string, unknown>).checkout_url as string;
          if (checkoutUrl) {
...
export async function gigSubmissionsCommand(taskId: string): Promise<void> {
  const config = requireConfig();
  const spinner = ora("Fetching gig submissions...").start();

  try {
    const resp = await apiGet<{ data?: EscrowSubmission[]; count?: number }>(
      `/api/v1/market/escrow/${taskId}/submissions`,
      config.api_key
    );
    ...
    const submissions = (resp.data as { data?: EscrowSubmission[] }).data ?? [];
    const count = (resp.data as { count?: number }).count ?? submissions.length;
