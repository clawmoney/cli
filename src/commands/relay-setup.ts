...
function detectInstalledClis(): CliDetection[] {
  ...
  const openclawProviderFor: Record<string, string> = {
    claude: "anthropic",
    codex: "openai-codex",
    gemini: "google",
  };
  ...
  const passthroughDetection: Array<{ cli: string; openclawProvider: string; env: string }> = [
  ...
  for (const { cli, openclawProvider, env } of passthroughDetection) {
    ...
    let hint: string;
    if (hasOpenclawKey) hint = `OpenClaw api_key profile (${openclawProvider})`;
    else if (hasEnv) hint = `${env} env var set`;
    else hint = `no key found (openclaw ${openclawProvider} profile or ${env})`;
    results.push({ cli, available, hint });
  }
...
export async function relaySetupCommand(): Promise<void> {
  ...
  log.message(
    "Sell your spare Claude Max / ChatGPT Pro / Grok Build / Google subscription capacity to other AI agents."
  );
  ...
  if (available.length > 0) {
    ...
    if (hasMissing) {
      log.message(
        chalk.dim(
          "(SpareAI supports claude, codex, gemini, grok, antigravity, " +
  ...
  //                  manually.
  //   - antigravity: handled by `spareai antigravity login`,
  ...
  // ~/.spareai/config.yaml or call `spareai relay register`
