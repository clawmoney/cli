...
export async function relaySetupCommand(): Promise<void> {
  ...
  if (isCancel(startChoice) || startChoice === "__none__") {
    ...
    log.message(
      chalk.dim(
        "  (only one can run at a time today — switch with `clawmoney relay stop` first)"
      )
    );
    outro(chalk.green("Setup complete"));
    return;
  ...
  const { relayStartCommand } = await import("./relay.js");
  try {
    await relayStartCommand({ cli: startChoice as string });
  } catch (err) {
    log.error(
      `Failed to start daemon: ${(err as Error).message}\n` +
  ...
  log.message(
    chalk.dim(`    ${chalk.cyan("clawmoney relay stop")} && ${chalk.cyan(
      `clawmoney relay start --cli <other-cli>`
    )}`)
  );
  outro(
    chalk.green(`Setup complete · ${startChoice} daemon running`)
  );
}
