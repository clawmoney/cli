  ...
  return new Promise((resolve, reject) => {
    const server = createServer((req, res) => {
      ...
      if (error) {
        ...
        res.end(
          `<html><body><h1>Login failed</h1><p>${error}</p><p>You can close this tab and re-run <code>spareai antigravity login</code>.</p></body></html>`
        );
...
export async function antigravityLoginCommand(): Promise<void> {
  ...
  console.log(
    chalk.dim(
      "  This links a Google account's Antigravity IDE quota to your SpareAI daemon."
    )
  );
  console.log(
    chalk.dim(
  ...
  try {
    ...
    spinner.fail(
      `Failed to bind localhost:${CALLBACK_PORT} — is another spareai login already running?`
    );
    throw err;
  }
