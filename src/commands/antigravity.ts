...
import {
  ...
  exchangeAntigravityAuthCode,
  fetchAntigravityUserEmail,
  storeNewAntigravityAccount,
  ANTIGRAVITY_ACCOUNTS_FILE,
  loadAccounts,
} from "../relay/upstream/antigravity-api.js";
...
function encodeState(verifier: string): string {
  return Buffer.from(JSON.stringify({ verifier, projectId: "" }), "utf8").toString(
    "base64url"
  );
}
...
function waitForCallback(): Promise<{ result: CallbackResult; server: Server }> {
  return new Promise((resolve, reject) => {
    const server = createServer((req, res) => {
      ...
      if (error) {
        ...
        res.end(
          `<html><body><h1>Login failed</h1><p>${error}</p><p>You can close this tab and re-run <code>clawmoney antigravity login</code>.</p></body></html>`
        );
    ...
    setTimeout(() => {
      reject(
        new Error(
          `Timed out waiting for OAuth callback after ${Math.round(
...
export async function antigravityLoginCommand(): Promise<void> {
  ...
  console.log(
    chalk.dim(
      "  This links a Google account's Antigravity IDE quota to your ClawMoney daemon."
    )
  );
  console.log(
    chalk.dim(
      "  Antigravity has a SEPARATE quota pool from Gemini CLI — use both to double your capacity."
    )
  );
  console.log("");
  ...
  let callbackPromise: Promise<{ result: CallbackResult; server: Server }>;
  try {
    callbackPromise = waitForCallback();
    spinner.succeed("Callback server ready");
  } catch (err) {
    spinner.fail(
      `Failed to bind localhost:${CALLBACK_PORT} — is another clawmoney login already running?`
    );
    throw err;
  }
  ...
  console.log("");
  console.log(
    chalk.dim("  (Attempting to open it automatically. If nothing opens, copy it manually.)")
  );
  ...
  try {
    ...
    waitSpinner.fail((err as Error).message);
    throw err;
  } finally {
  ...
  let tokens: Awaited<ReturnType<typeof exchangeAntigravityAuthCode>>;
  try {
    ...
    exchangeSpinner.fail((err as Error).message);
    throw err;
  }
  ...
  const storeSpinner = ora(
    "Resolving project ID and persisting account..."
  ).start();
  try {
    ...
    storeSpinner.succeed(
      `Saved to ${ANTIGRAVITY_ACCOUNTS_FILE} (project=${account.project_id})`
    );
  } catch (err) {
    storeSpinner.fail((err as Error).message);
    throw err;
  }
  ...
  console.log(
    chalk.dim(
      "  Next: register a model and start the daemon. Example:\n" +
...
export async function antigravityStatusCommand(): Promise<void> {
  ...
  for (let i = 0; i < file.accounts.length; i++) {
    ...
    const expiryStr = a.expiry_ms
      ...
      : "unknown";
    console.log(
      `  ${i === 0 ? chalk.bold("●") : " "} ${a.email ?? "(email unknown)"}`
    );
