...
function failOption(flag: string, detail: string): never {
  console.error(chalk.red(`Invalid ${flag}: ${detail}.`));
  process.exit(1);
}

function getOptionText(raw: string | undefined, flag: string, bounds: NumberBounds): string {
  if (raw == null) {
    if (bounds.defaultValue != null) return String(bounds.defaultValue);
...
function enforceBounds(value: number, flag: string, bounds: NumberBounds): void {
...
export function parseJsonObjectOption(raw: string, flag: string): Record<string, unknown> {
  let parsed: unknown;
  try {
