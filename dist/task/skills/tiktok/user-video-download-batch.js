...
import { isYtDlpInstalled, ytdlpTikTokUserBatchDownload, } from "./_ytdlp.js";
/**
 ...
 */
export const tkUserVideoDownloadBatchSkill = {
    price_usd: 0.005,
    async run(input, ctx) {
        const i = (input ?? {});
        const user = i.user || i.uniqueId || i.secUid || i.url;
        if (!user)
            throw new Error("missing 'secUid' (or user/uniqueId/url)");
        ctx.report({
            stage: "launching",
            percent: 5,
