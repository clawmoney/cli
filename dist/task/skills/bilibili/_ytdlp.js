...
export async function ytdlpBilibiliDownload(input) {
    ...
    const requestedFormats = (info.requested_formats || [])
        .filter((f) => f && f.url)
        .map(mapFormat);
    const bestVideo = requestedFormats.find((f) => f.has_video && !f.has_audio)
        ?? formats
            .filter((f) => f.has_video)
            .sort((a, b) => (b.height - a.height) || (b.bitrate - a.bitrate))[0];
    const bestAudio = requestedFormats.find((f) => f.has_audio && !f.has_video)
        ?? formats
