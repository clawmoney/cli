 ...
 */
export async function ytdlpTranscript(videoId, langPref = "en") {
 ...
 */
export async function ytdlpStreamingData(videoId) {
    if (!/^[A-Za-z0-9_-]{11}$/.test(videoId)) {
        throw new Error(`invalid videoId: ${videoId}`);
    }
    if (!(await isYtDlpInstalled())) {
    ...
    const { stdout, stderr, code } = await run("yt-dlp", ["--dump-json", "--skip-download", "--no-warnings", `https://www.youtube.com/watch?v=${videoId}`], 45_000);
    if (code !== 0) {
        throw new Error(`yt-dlp exited ${code}: ${stderr.split("\n").slice(-5).join(" | ").slice(0, 400)}`);
    }
    ...
    const audioBitrateBand = (abr) => abr >= 192 ? "AUDIO_QUALITY_HIGH" : abr >= 96 ? "AUDIO_QUALITY_MEDIUM" : "AUDIO_QUALITY_LOW";
    const mapFormat = (f) => {
