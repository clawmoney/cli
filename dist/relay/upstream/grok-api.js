...
function grokClientHeaders(extra = {}) {
    return {
...
export function __resetGrokApiForTests() {
    testHooks = {};
...
function readJsonRecord(path, label) {
    ...
    try {
        ...
        throw new Error(`Grok ${label} could not be read at ${path}: ${err.message}`);
    }
    const record = asRecord(parsed);
    if (!record)
        throw new Error(`Grok ${label} at ${path} is not a JSON object`);
    return record;
}
...
function readGrokCredentials() {
    const path = authFile();
    if (!existsSync(path)) {
        throw new Error(`Grok OAuth authentication missing at ${path}. Run \`grok login\` first.`);
    }
    ...
    if (!selected) {
        throw new Error(`Grok OAuth authentication at ${path} has no cached access key. Run \`grok login\` first.`);
    }
    return selected;
}
...
function unwrapModelInfo(value) {
    const record = asRecord(value);
    if (!record)
        return null;
    return asRecord(record.info) ?? record;
}
function findModelInfo(root, model) {
    const models = root.models;
    if (Array.isArray(models)) {
        for (const entry of models) {
            const info = unwrapModelInfo(entry);
            if (info && modelIdentity(info) === model)
                return info;
        }
    }
    else {
    ...
    if (typeof (root.base_url ?? root.baseUrl) === "string" &&
        (!modelIdentity(root) || modelIdentity(root) === model)) {
        return root;
    }
    return null;
}
function resolveModelEndpoint(model) {
    const path = modelsCacheFile();
    if (!existsSync(path)) {
        throw new Error(`Grok model cache missing at ${path}. Run \`grok models\` first.`);
    }
    const root = readJsonRecord(path, "model cache");
    const info = findModelInfo(root, model);
    if (!info) {
        throw new Error(`Grok model ${JSON.stringify(model)} is not present in ${path}. Run \`grok models\` to refresh the catalog.`);
    }
    const backend = info.api_backend ?? info.apiBackend;
    if (typeof backend === "string" && backend.toLowerCase() !== "responses") {
        throw new Error(`Grok model ${JSON.stringify(model)} uses unsupported api_backend=${JSON.stringify(backend)} (Responses required)`);
    }
    const authScheme = info.auth_scheme ?? info.authScheme;
    if (typeof authScheme === "string" &&
        authScheme.toLowerCase() !== "bearer") {
        throw new Error(`Grok model ${JSON.stringify(model)} uses unsupported auth_scheme=${JSON.stringify(authScheme)} (Bearer required)`);
    }
    ...
    if (typeof rawBase !== "string" || !rawBase.trim()) {
        throw new Error(`Grok model ${JSON.stringify(model)} has no Responses base_url in ${path}`);
    }
    ...
    if (parsed.host !== OFFICIAL_GROK_PROXY_HOST ||
        parsed.username !== "" ||
        parsed.password !== "" ||
        parsed.pathname !== "/v1" ||
        parsed.search !== "" ||
        parsed.hash !== "") {
        throw new Error(`Grok Responses base_url must be exactly https://${OFFICIAL_GROK_PROXY_HOST}/v1`);
    }
    return {
        baseUrl,
...
function isRefreshableModelCacheError(err) {
    const message = err instanceof Error ? err.message : String(err);
    return (message.includes("Grok model cache missing") ||
...
function resolvePathBinary(name) {
...
export function inspectGrokLocalState(model = DEFAULT_MODEL) {
    ...
    try {
        ...
        if (!authFresh)
            authProblem = "cached OAuth access key expired or near expiry";
    }
    catch (err) {
        authProblem = err.message;
    }
    try {
    ...
    const available = Boolean(binaryPath && authPresent && (modelCacheValid || modelCacheRefreshable));
    let hint;
    if (!binaryPath) {
...
function runOfficialModels(binaryPath) {
    if (testHooks.runModels)
        return testHooks.runModels(binaryPath);
    return new Promise((resolve, reject) => {
        execFile(binaryPath, ["models"], {
            ...
            const code = err.code;
            reject(new Error(`Grok OAuth refresh via official CLI failed${code ? ` (${code})` : ""}. Run \`${binaryPath} models\` manually.`));
        });
    });
}
let refreshInflight = null;
async function refreshViaOfficialCli(rejectedKey) {
    ...
    if (rejectedKey) {
        try {
            ...
            if (current.accessKey !== rejectedKey &&
    ...
    if (!refreshInflight) {
        refreshInflight = (async () => {
            ...
            if (!binary) {
                throw new Error(`Grok OAuth refresh requires the official CLI (checked GROK_BIN, ${join(grokHome(), "bin", "grok")}, and PATH)`);
            }
            ...
            if (refreshed.expiresAt <= nowMs() + AUTH_REFRESH_SKEW_MS) {
                throw new Error(`Grok OAuth refresh via official CLI did not produce a valid cached access key. Run \`${binary} login\` and retry.`);
            }
            return refreshed;
        })().finally(() => {
            refreshInflight = null;
...
async function getFreshCredentials() {
    ...
    if (cached.expiresAt > nowMs() + AUTH_REFRESH_SKEW_MS)
        return cached;
...
let rateGuard = null;
export function configureGrokRateGuard(config) {
    const mapped = {};
...
export function getGrokRateGuardSnapshot() {
...
async function throwHttpError(response) {
    ...
    if (response.status === 401 || response.status === 403) {
        throw new Error(`Grok authentication unauthorized (upstream ${response.status}): ${snippet}`);
    }
    throw new Error(`Grok upstream ${response.status}: ${snippet}`);
}
// ── Preflight ───────────────────────────────────────────────────────────
export async function preflightGrokApi(config) {
    configureDispatcher();
    configureGrokRateGuard(config);
    const binary = findGrokBinary();
    if (!binary) {
        throw new Error(`Official Grok CLI not found (checked GROK_BIN, ${join(grokHome(), "bin", "grok")}, and PATH)`);
    }
    ...
    try {
        ...
        if (!isRefreshableModelCacheError(err))
            throw err;
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
        ...
        await response.text();
        logger.info(`[grok-api] preflight OK (binary=${binary}, expires_in=${Math.max(0, Math.floor((creds.expiresAt - nowMs()) / 1000))}s)`);
        return;
    }
}
...
function normalizeUsage(value) {
    ...
    return {
        ...
        cacheReadTokens: numberValue(inputDetails.cached_tokens ?? usage.cache_read_input_tokens),
...
function applyResponseObject(acc, value, allowFinalText) {
    const response = asRecord(value);
    if (!response)
        return;
    if (typeof response.id === "string")
        acc.sessionId = response.id;
    if (typeof response.model === "string")
        acc.model = response.model;
    if (response.usage)
        acc.usage = normalizeUsage(response.usage);
    if (allowFinalText) {
...
function consumeResponseEvent(acc, payload, eventName) {
    const event = asRecord(payload);
    if (!event)
        return;
    const type = (typeof event.type === "string" && event.type) || eventName || "message";
    ...
    if (typeof event.id === "string" && !acc.sessionId)
        acc.sessionId = event.id;
    if (typeof event.model === "string")
        acc.model = event.model;
    if (event.usage)
        acc.usage = normalizeUsage(event.usage);
    ...
    const responseStatus = typeof responseRecord?.status === "string"
    ...
    if (type === "response.failed" ||
        ...
        throw new Error(`Grok Responses error: ${errorMessage(event.error ?? responseRecord?.error ?? event)}`);
    }
    if (type === "response.incomplete" || responseStatus === "incomplete") {
        throw new Error(`Grok Responses incomplete: ${errorMessage(responseRecord?.incomplete_details ?? event.incomplete_details ?? event)}`);
    }
...
async function parseSseResponse(response, model, onRawEvent) {
    const reader = response.body?.getReader();
    if (!reader)
        throw new Error("Grok Responses stream returned an empty body");
    const acc = makeAccumulator(model);
    const decoder = new TextDecoder();
    let buffer = "";
    const consumeFrame = (rawFrame) => {
        if (!rawFrame.trim())
            return;
        ...
        try {
            ...
            throw new Error(`Grok Responses stream contained invalid JSON: ${data.slice(0, 160)}`);
        }
    ...
    if (buffer.trim())
        consumeFrame(buffer);
    if (!acc.completed) {
        throw new Error("Grok Responses stream ended before response.completed (truncated response)");
    }
    return acc;
}
async function parseJsonResponse(response, model) {
    let payload;
    try {
        payload = JSON.parse(await response.text());
    }
    catch (err) {
        throw new Error(`Grok Responses returned invalid JSON: ${err.message}`);
    }
    ...
    if (status === "incomplete" || status === "failed") {
        throw new Error(`Grok Responses ${status}: ${errorMessage(record?.incomplete_details ?? record?.error ?? record)}`);
    }
    const acc = makeAccumulator(model);
    applyResponseObject(acc, payload, true);
    return acc;
}
function hasResponsesInput(body) {
...
function validatePassthroughSafety(body, stream) {
    ...
    if (body.previous_response_id != null || body.conversation != null) {
        throw new Error("Grok stored response/conversation continuation is not supported; resend full prior context in input");
    }
    ...
    if (body.tools.length > 0 && !stream) {
        throw new Error("Grok function tools require stream:true so typed function_call events can pass through intact");
    }
    for (const rawTool of body.tools) {
        ...
        if (PAID_SERVER_TOOL_TYPES.has(type)) {
            throw new Error(`Grok server-side tool ${JSON.stringify(type)} is disabled because it incurs separate provider charges`);
        }
        throw new Error(`Unsupported Grok tool type ${JSON.stringify(type || "<missing>")}; relay only allows streamed function tools`);
    }
}
function buildRequestBody(opts) {
    ...
    if (!prompt)
        throw new Error("Empty Grok prompt");
    return {
        body: {
            model: opts.model,
            input: [
...
export async function callGrokApi(opts) {
    configureDispatcher();
    if (!rateGuard)
        configureGrokRateGuard();
    return rateGuard.run(() => doCallGrokApi(opts));
}
async function doCallGrokApi(opts) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
        ...
        const cacheCreation = Math.min(Math.max(0, totalInput - cacheRead), acc.usage.cacheCreationTokens);
        const baseInput = Math.max(0, totalInput - cacheRead - cacheCreation);
        const cost = calculateCost(acc.model || opts.model, baseInput, acc.usage.outputTokens, cacheCreation, cacheRead);
        rateGuard?.recordSpend(cost.apiCost);
        logger.info(`[grok-api] OK model=${acc.model || opts.model} in=${baseInput} out=${acc.usage.outputTokens} cache_read=${cacheRead}`);
        return {
            text: acc.text,
