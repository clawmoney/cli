...
function grokCliVersion() {
    try {
        if (existsSync(GROK_VERSION_FILE)) {
            const raw = JSON.parse(readFileSync(GROK_VERSION_FILE, "utf-8"));
            if (raw.version && raw.version.trim())
...
function loadAuthFile() {
    if (!existsSync(GROK_AUTH_FILE)) {
        throw new Error(`Grok credentials not found at ${GROK_AUTH_FILE}. Run \`grok login\` first.`);
    }
    const parsed = JSON.parse(readFileSync(GROK_AUTH_FILE, "utf-8"));
...
function pickAuthEntry(store) {
    ...
    throw new Error(`${GROK_AUTH_FILE} has no usable key. Run \`grok login\` and confirm the file has an OIDC entry.`);
}
...
function persistCreds(creds) {
    if (!creds.entryKey || creds.expiresAt === Infinity)
        return;
...
async function refreshGrokToken(current) {
    ...
    if (current.principalType)
        body.set("principal_type", current.principalType);
    if (current.principalId)
        body.set("principal_id", current.principalId);
    ...
    try {
        ...
        logger.warn(`[grok] persist after refresh failed: ${err.message}`);
    }
    return next;
}
...
async function getFreshCreds() {
    ...
    if (cachedCreds.expiresAt === Infinity)
        return cachedCreds;
    if (cachedCreds.expiresAt - Date.now() > REFRESH_SKEW_MS)
        return cachedCreds;
...
function grokIdentityHeaders(opts) {
    ...
    if (opts.userId)
        headers["x-grok-user-id"] = opts.userId;
    return headers;
}
async function doCall(opts) {
    ...
    if (!reader)
        throw new Error("grok upstream returned empty body");
