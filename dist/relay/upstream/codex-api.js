...
function loadCodexFingerprint() {
    ...
    logger.info(`[codex-api] fingerprint loaded (ua=${cachedFingerprint.user_agent}, originator=${cachedFingerprint.originator})`);
    return cachedFingerprint;
}
// ── JWT exp decode ──
//
...
function parseCodexRateLimitHeaders(headers) {
    function getMs(resetAfterSecondsHeader) {
...
export async function preflightCodexApi(config) {
    ...
    await getFreshCreds();
    logger.info(`[codex-api] preflight OK (account_id=***${cachedCreds?.accountId?.slice(-6) ?? "?"}, expires=${new Date(cachedCreds?.expiresAt ?? 0).toISOString()})`);
}
// ── Responses API request builder ──
function buildRequestBody(prompt, model, _maxTokens) {
    const body = {
        model,
        input: [
            {
...
function parseCodexSSE(sseBody, fallbackModel) {
...
async function doCallCodexApi(opts) {
    ...
    while (true) {
        ...
        if (resp.ok) {
            ...
            if (rateGuard) {
                const cost = calculateCost(opts.model, result.usage.input_tokens, result.usage.output_tokens, result.usage.cache_creation_tokens, result.usage.cache_read_tokens);
                rateGuard.recordSpend(cost.apiCost);
                result.costUsd = cost.apiCost;
            }
            logger.info(`[codex-api] OK model=${parsed.model} in=${parsed.inputTokens} out=${parsed.outputTokens} cache_read=${parsed.cacheReadTokens}`);
            return result;
        }
        ...
        if (isTransient && transientAttempt < MAX_TRANSIENT_RETRIES) {
            ...
            logger.warn(`[codex-api] ${resp.status} from upstream (attempt ${transientAttempt + 1}/${MAX_TRANSIENT_RETRIES + 1}), retrying in ${Math.round(backoffMs)}ms — ${errText.slice(0, 200)}`);
