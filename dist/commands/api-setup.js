...
import { intro, outro, multiselect, text, confirm, spinner, isCancel, log, note, } from "@clack/prompts";
...
function formatProjectedEarnings(selected) {
    let monthly = 0;
    const lines = [];
    for (const slug of selected) {
        ...
        lines.push(`  ${p.label.padEnd(16)}  $${p.pricePerCall.toFixed(4)}/call  ×  ${p.defaultDailyVolume}/day  →  $${monthlyPlatform.toFixed(2)}/mo`);
    }
    lines.push("");
    lines.push(`  ${chalk.bold("Projected total")}: ${chalk.green(`~$${monthly.toFixed(2)}/mo`)} ${chalk.dim(`(${Math.round(OPERATOR_SHARE * 100)}% operator share)`)}`);
    return lines.join("\n");
}
export async function apiSetupCommand(opts = {}) {
    ...
    log.message([
        ...
        "",
        chalk.dim("Customer hits SpareAPI → ClawMoney routes to your agent → bnbot opens"),
        chalk.dim("the target site in real Chrome → JSON returns. You keep 70% per call."),
    ].join("\n"));
    ...
    log.success(`Saved ${platforms.length} platform${platforms.length === 1 ? "" : "s"} to ~/.clawmoney/config.yaml`);
    ...
    if (!isCancel(checkBnbot) && checkBnbot) {
        ...
        try {
            ...
            s.stop(chalk.yellow(`bnbot status check failed: ${err.message}`));
            log.message([
